...
def linearMethod(train_data,train_target,model):
    ...
    if model == 'Ridge':
        linear = lm.RidgeCV(normalize=True)#岭回归交叉验证，对惩罚系数进行调优
    elif model == 'Lasso':
        linear = lm.LassoCV(normalize=True)#套索回归交叉验证，对惩罚系数进行调优
        #linear = lm.Lasso(alpha = reg.alpha_,normalize=True)#套索回归，设置normalize参数对特征进行缩放，设置alpha惩罚系数
    elif model == 'ElasticNet':
        linear = lm.ElasticNetCV(normalize=True)#弹性网络回归交叉验证，对惩罚系数进行调优
        #linear = lm.ElasticNet(alpha = reg.alpha_,normalize=True)#弹性网络回归，设置normalize参数对特征进行缩放，设置alpha惩罚系数
    else:
        linear = lm.LinearRegression(normalize=True)#最小二乘法，设置normalize参数对特征进行缩放
